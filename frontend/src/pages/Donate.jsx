...
const Donate = () => {
  ...
  React.useEffect(() => {
    ...
    // 3. In production only, clear console and override to prevent logging sensitive details
  ...
  const handlePayment = async (amount) => {
    ...
    if (nameError || emailError || contactError || panError) {
      setFormMessage({
        type: "error",
        text: "Please fix the errors in your details before proceeding.",
      });
    ...
    if (!res) {
      setFormMessage({
        type: "error",
        text: "Payment SDK failed to load. Are you online?",
      });
      setLoading(false);
      return;
    }
    ...
    try {
      ...
      const orderResponse = await fetch(
        `${import.meta.env.VITE_API_BASE_URL || ""}/api/payment/order`,
        {
          ...
          body: JSON.stringify({
            ...
            pan_number: donorPan.toUpperCase(),
          }),
        },
      );
      ...
      if (!orderData.success) {
        setFormMessage({
          type: "error",
          text: orderData.message || orderData.error || "Server error creating order. Please try again later.",
        });
        setLoading(false);
        return;
      }
      ...
      const keyResponse = await fetch(
        `${import.meta.env.VITE_API_BASE_URL || ""}/api/payment/key`,
      );
      ...
      const options = {
        ...
        handler: async function (response) {
          ...
          if (verifyData.success) {
            setFormMessage({
              type: "success",
              text: "Payment Successful! Thank you for your generous donation.",
            });
            window.scrollTo({ top: 300, behavior: "smooth" });
          } else {
            setFormMessage({
        ...
        modal: {
          ondismiss: function () {
            fetch(
              `${import.meta.env.VITE_API_BASE_URL || ""}/api/payment/failure`,
              {
                method: "POST",
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify({
    ...
    <div className="bg-gray-50 min-h-screen pb-20">
      ...
      <div className="bg-primary text-white py-16 text-center rounded-b-[3rem] shadow-xl relative overflow-hidden">
        ...
        <div className="container mx-auto px-4 relative z-10">
          ...
          <p className="text-xl text-white/80 max-w-2xl mx-auto">
            Your small contribution can create a massive impact.
          </p>
        </div>
      </div>
      ...
      <div className="container mx-auto px-4 -mt-10">
        <div className="bg-white p-8 rounded-xl shadow-xl max-w-4xl mx-auto border border-gray-100">
          {formMessage.text && (
            <div
              className={`mb-8 p-4 rounded-xl border font-medium flex items-center gap-3 ${formMessage.type === "success" ? "bg-green-50 border-green-200 text-green-700" : "bg-red-50 border-red-200 text-red-700"}`}
            >
              {formMessage.type === "success" ? (
                <CheckCircle size={20} />
              ) : (
                <div className="w-5 h-5 rounded-full border-2 border-red-500 text-red-500 font-bold flex items-center justify-center text-xs">
                  !
                </div>
              )}
              {formMessage.text}
            ...
            <div className="grid md:grid-cols-2 gap-4">
              <div>
                <input
                  ...
                  className={`w-full p-4 bg-white rounded-xl font-medium outline-none border focus:ring-1 ${formErrors.name ? "border-red-300 focus:border-red-500 focus:ring-red-500" : "border-gray-200 focus:border-primary focus:ring-primary"}`}
                />
                {formErrors.name && (
              ...
              <div>
                <input
                  ...
                  className={`w-full p-4 bg-white rounded-xl font-medium outline-none border focus:ring-1 ${formErrors.email ? "border-red-300 focus:border-red-500 focus:ring-red-500" : "border-gray-200 focus:border-primary focus:ring-primary"}`}
                />
                {formErrors.email && (
                  <p className="text-red-500 text-sm mt-1 px-1">
                    {formErrors.email}
              ...
              <div>
                <input
                  ...
                  className={`w-full p-4 bg-white rounded-xl font-medium outline-none border focus:ring-1 ${formErrors.contact ? "border-red-300 focus:border-red-500 focus:ring-red-500" : "border-gray-200 focus:border-primary focus:ring-primary"}`}
                />
                {formErrors.contact && (
                  <p className="text-red-500 text-sm mt-1 px-1">
                    {formErrors.contact}
          ...
          <div className="grid md:grid-cols-3 gap-6 mb-8">
            {programs.map((prog) => (
              <div
                key={prog.id}
                onClick={() => handlePayment(prog.amount)}
                className="border-2 border-transparent hover:border-primary bg-gray-50 p-6 rounded-xl cursor-pointer transition-all hover:bg-primary/10 group text-center flex flex-col h-full"
              >
                ...
                <div className="text-lg font-bold text-primary mt-auto">
                  ₹{Number(prog.amount).toLocaleString()}
                </div>
              </div>
            ))}
          </div>
          ...
          <div className="text-center">
            ...
            <div className="flex gap-4 justify-center max-w-md mx-auto mb-8">
              <span className="p-4 bg-gray-100 rounded-l-xl font-bold text-gray-500 text-xl">
                ₹
              </span>
              <input
                type="number"
            ...
            <button
              onClick={() => {
                if (customAmount && customAmount >= 99)
                  ...
                  setFormMessage({
                    type: "error",
                    text: "Please enter a valid amount (Minimum ₹99)",
                  });
              }}
              disabled={loading}
        ...
        <div className="mt-16 grid md:grid-cols-3 gap-8 max-w-5xl mx-auto text-center">
          ...
          <div className="p-6">
            ...
            <p className="text-gray-600">
              We use SSL encryption to ensure your data is safe.
            </p>
          </div>
          <div className="p-6">
            ...
            <p className="text-gray-600">
              Receive regular updates on how your funds are used.
            </p>
          </div>
        </div>
      </div>
